...
function applyFilters() {
  ...
  if (baixoOnly)  filtered = filtered.filter(p => p.stockAtual <= p.stockMinimo);
...
function renderTable(pecas) {
  ...
  tbody.innerHTML = pecas.map(p => {
    const low = p.stockAtual <= p.stockMinimo;
    return `
      <tr>
        ...
        <td>
          <span class="${low ? 'stock-low' : 'stock-ok'}">${p.stockAtual}</span>
        ...
        <td>
          <div class="table-actions">
            ...
            <button class="btn btn-warning btn-sm" onclick="openSaida(${p.id}, '${escapeHtml(p.designacao)}', ${p.stockAtual})" title="Saída Stock" ${p.stockAtual <= 0 ? 'disabled' : ''}>📤</button>
...
function openEditPeca(id) {
  ...
  document.getElementById('p-stock').value       = p.stockAtual ?? 0;
...
async function submitPeca(e) {
  ...
  const payload = {
    ...
    stockAtual:    parseInt(document.getElementById('p-stock').value) || 0,
    ...
    localizacao:   document.getElementById('p-localizacao').value.trim() || null,
...
/* ── Saída de Stock ── */
function openSaida(pecaId, nome, stockAtual) {
  document.getElementById('saida-peca-id').value           = pecaId;
  document.getElementById('saida-peca-nome').textContent   = nome;
  document.getElementById('saida-stock-atual').textContent = `Stock disponível: ${stockAtual}`;
  document.getElementById('saida-qty').value   = 1;
  document.getElementById('saida-qty').max     = stockAtual;
