...
function renderTable(viaturas) {
  ...
  tbody.innerHTML = viaturas.map(v => `
    <tr>
      ...
      <td>${escapeHtml(v.clienteNome || v.cliente?.nome || '—')}</td>
      <td>
...
function openEditViatura(id) {
  ...
  document.getElementById('viat-vin').value        = v.numeroChassis || ''; // <-- Corrigido para numeroChassis
  ...
  document.getElementById('viat-obs').value        = v.observacoes || ''; // <-- Carrega as observacoes
...
async function submitViatura(e) {
  ...
  // LER OS VALORES DOS CAMPOS DE FORMA SEGURA (evita envio de strings vazias para ints)
  ...
  const payload = {
    ...
    numeroChassis: document.getElementById('viat-vin').value.trim() || null, // <-- Ajustado
    ...
    observacoes:   document.getElementById('viat-obs').value.trim() || null, // <-- Ajustado
