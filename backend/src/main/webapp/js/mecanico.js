...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 2. Autenticação e Mensagem de Boas-Vindas
  const user = getCurrentUser();
  if (user) {
    const userNameEl = document.getElementById('top-user-name');
    if (userNameEl) {
      // Usa o nome guardado no localStorage (adaptado para name ou nome consoante o teu modelo)
...
/* ── 1. NAVEGAÇÃO ENTRE PÁGINAS E CORREÇÃO DO BUG VOLTAR ── */
function voltarGlobal() {
  ...
  if (viewDetails && !viewDetails.classList.contains('hidden')) {
    ...
    // Se já estiver na Fila de Trabalho:
    const user = getCurrentUser();
    // Se for MANAGER, volta para a visão global de gestão
    if (user && user.role === 'MANAGER') {
      window.location.href = 'dashboard.html'; 
    } else {
      // Se for MECANICO, como não tem outra dashboard, ou faz logout ou vai para o index.
      // Adicionamos um bypass na URL caso o teu index.html tenha auto-redirecionamento.
...
function backToDashboard() {
  ...
  // 5. Se o cronómetro estiver a contar ao retroceder, pausa-o para guardar automaticamente
  if (timerRunning) {
    toggleTimer(); 
  }
...
/* ── PÁGINA 1: DASHBOARD E TABELA ── */
/* ── 3. CARREGAR FILA DE TRABALHO COM DIFERENCIAÇÃO DE PERFIS ── */
async function loadMyRepairs() {
  ...
  try {
    ...
    if (user.role === 'MANAGER' || user.role === 'ADMIN') {
      // Administrador: vê todas as reparações da oficina
      data = await api.getReparacoes() || [];
    } else {
      // Mecânico: vê apenas as reparações a ele atribuídas
      data = await api.getReparacoesMecanico(user.id) || [];
    }
    
    // Atualiza KPIs (Contadores do topo)
...
function renderRepairList() {
  ...
  if (!allRepairs.length) {
    tbody.innerHTML = `<tr><td colspan="6" class="text-center" style="padding: 2rem;">🎉 Sem trabalho pendente na fila.</td></tr>`;
    return;
  }
...
/* ── PÁGINA 2: TRANSIÇÃO E RECUPERAÇÃO DO CRONÓMETRO ── */
/* ── TRANSIÇÃO E RECUPERAÇÃO DO CRONÓMETRO ── */
async function selectReparacao(id) {
  ...
  try {
    ...
    // 5. Vai buscar o tempo em segundos que estava guardado no browser para ESTA reparação
...
function renderDetailPanel() {
  ...
  // 4. Lógica do Stepper ATUALIZADA (Apenas 4 passos diretos)
  const s = r.estado;
...
function openOpModal(opId) {
  ...
  if (opId && currentRepair) {
    ...
    if (op) {
      ...
      currentOpId = opId; // Vincula timer
      
      // Se vinculou uma operação nova e o timer estava parado a zeros, garante que a UI o mostra
      renderDetailPanel(); 
    }
  } else {
    currentOpId = null;
...
/* ========================================================
   NOVO CRONÓMETRO PERSISTENTE (LOCALSTORAGE)
   ======================================================== */

// Obtém a chave única para guardar o tempo desta reparação
function getTimerKey() {
  return 'sgo_timer_' + (currentRepair?.id || 'unknown');
}

// Inicia ou Pausa o Cronómetro
function toggleTimer() {
  if (timerRunning) {
    ...
    // AÇÃO: RETOMAR/INICIAR
    timerRunning = true;
    timerInterval = setInterval(() => {
      timerSeconds++;
      
      // Atualiza o display no ecrã
      const display = document.getElementById('stopwatch-display');
      if (display) display.textContent = formatStopwatch(timerSeconds);
      
      // Guarda o tempo a cada segundo na memória do browser
      localStorage.setItem(getTimerKey(), timerSeconds);
    }, 1000);
  }
  
  // Guarda o estado final (caso pausemos no meio do segundo)
  localStorage.setItem(getTimerKey(), timerSeconds);
  
  // Atualiza o botão visualmente (Verde -> Amarelo)
...
// Guarda permanentemente o tempo na Base de Dados e limpa o LocalStorage
function saveTimeToOperation() {
  // Se estiver a correr, pausa para ter a certeza que não perdemos o último segundo
...
async function concluirReparacao() {
  ...
  if (timerRunning) toggleTimer(); // Pausa antes de fechar
...
// Chave única para guardar o tempo associado à reparação atual
function getTimerKey() {
  return 'sgo_timer_' + (currentRepair?.id || 'unknown');
}
...
function toggleTimer() {
  if (timerRunning) {
    ...
    timerInterval = setInterval(() => {
      ...
      // Guarda continuamente na memória (se falhar a luz ou sair da página, fica guardado)
      localStorage.setItem(getTimerKey(), timerSeconds);
    }, 1000);
  }
  
  // Assegura que o estado final fica guardado ao pausar
  localStorage.setItem(getTimerKey(), timerSeconds);
  
  // Atualiza o aspeto do botão dinamicamente
  const btn = document.getElementById('btn-toggle-timer');
  if (btn) {
    btn.className = `btn btn-sm ${timerRunning ? 'btn-warning' : 'btn-success'}`;
    btn.innerHTML = timerRunning ? '⏸ Pausar' : '▶ Retomar';
...
// Quando o tempo é guardado no backend com sucesso (função saveTimeToOperation)
// Deve-se limpar o localStorage com:
// timerSeconds = 0;
// localStorage.removeItem(getTimerKey());
// document.getElementById('stopwatch-display').textContent = formatStopwatch(0);